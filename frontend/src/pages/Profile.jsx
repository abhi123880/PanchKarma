...
const Profile = ({ currentUser, setCurrentUser }) => {
  ...
  // NEW: Always fetch latest user data (with avatar!) on mount/refresh
  useEffect(() => {
    if (currentUser && currentUser.id) {
      const token = localStorage.getItem('token');
      fetch(`https://panchkarma.onrender.com/api/user/${currentUser.id}`, {
  ...
  }, [currentUser?.id]); // only when ID changes[1][3]
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // RECOMMENDED: Refetch user to update avatar and all latest info
        fetch(`https://panchkarma.onrender.com/api/user/${currentUser.id}`, {
          headers: { "Authorization": `Bearer ${token}` },
        })
          .then(res => res.json())
