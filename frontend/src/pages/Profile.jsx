  ...
  useEffect(() => {
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setFileUploadError(false);
        setFilePerc(100); // Simulate progress
        localStorage.setItem('avatar', base64); // Optional local storage for refresh fallback
      };
      reader.onerror = () => {
        setFileUploadError(true);
        setFilePerc(0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (typeof setCurrentUser === "function") {
          setCurrentUser(data.user); // should include updated avatar
        }

        // Sync updated avatar with local storage
        if (data.user?.avatar) {
